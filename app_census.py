...
# Version: v2.1 - FORCE DEPLOYMENT REFRESH
# Configure to reduce auto-refresh
...
class BusStopsTemperatureAnalyzer:
    ...
    def interpolate_temperature_at_stops(self, bus_stops_df, census_temperature_df):
        ...
        try:
            # Clean input data
            bus_clean = bus_stops_df.dropna(subset=['lat', 'lng']).copy()
            census_clean = census_temperature_df.dropna(subset=['lat', 'lng', 'mean_temp']).copy()
            
            if bus_clean.empty or census_clean.empty:
                # Fallback to default temperature
                bus_stops_df['estimated_temp'] = 85.0
                return bus_stops_df
            
            # Create coordinate arrays
            bus_coords = bus_clean[['lat', 'lng']].values
            census_coords = census_clean[['lat', 'lng']].values
            ...
            for i, stop_distances in enumerate(distances):
                ...
                if len(nearby_indices) > 0:
                    nearby_temps = census_clean.iloc[nearby_indices]['mean_temp'].values
                    nearby_dists = stop_distances[nearby_indices]
                    
                    # Remove any NaN temperatures
                    valid_mask = ~np.isnan(nearby_temps)
                    if np.any(valid_mask):
                        nearby_temps = nearby_temps[valid_mask]
                        nearby_dists = nearby_dists[valid_mask]
                        ...
                        temperatures.append(85.0)  # Default temperature
                else:
                    # Use nearest point if none within radius
                    nearest_idx = np.argmin(stop_distances)
                    nearest_temp = census_clean.iloc[nearest_idx]['mean_temp']
                    temperatures.append(nearest_temp if not np.isnan(nearest_temp) else 85.0)
            
            # Map temperatures back to original dataframe
            temp_mapping = dict(zip(bus_clean.index, temperatures))
            bus_stops_df['estimated_temp'] = bus_stops_df.index.map(temp_mapping).fillna(85.0)
            
        except Exception as e:
            st.warning(f"Temperature interpolation failed, using default values: {str(e)}")
            bus_stops_df['estimated_temp'] = 85.0
    ...
    def calculate_heat_exposure_risk(self, bus_stops_df):
        ...
        try:
            def calculate_risk(row):
                # Get base temperature, default to 85 if missing
                base_risk = row.get('estimated_temp', 85.0)
                if pd.isna(base_risk):
                    base_risk = 85.0
                
                # Adjust risk based on infrastructure
                if row.get('Shelter', False):
                    base_risk -= 5  # Shelter provides significant cooling
                elif row.get('Bench', False):
                    base_risk -= 2  # Bench provides some relief
                
                # Higher frequency stops = more exposure time
                freq_am = row.get('Frequency_AM_Peak', 0) or 0
                freq_mid = row.get('Frequency_Midday', 0) or 0
                freq_pm = row.get('Frequency_PM_Peak', 0) or 0
                freq_eve = row.get('Frequency_Evening', 0) or 0
                
                avg_frequency = (freq_am + freq_mid + freq_pm + freq_eve) / 4
            ...
            st.warning(f"Risk calculation failed, using default values: {str(e)}")
            bus_stops_df['heat_risk_score'] = 80.0
            bus_stops_df['risk_category'] = 'Moderate'
            
        return bus_stops_df
    
    def create_bus_stops_map_v2(self, bus_stops_df, time_period='all', census_data=None):
        """Create interactive map of bus stops with heat risk - ULTRA SIMPLE VERSION"""
        st.write("🚌 Creating bus stops map...")
        
        # Return simple Plotly Express map - no custom traces that can fail
        try:
            if bus_stops_df.empty:
                st.warning("No bus stop data available")
                return px.scatter_mapbox(
                    pd.DataFrame({'lat': [35.19], 'lng': [-80.79]}),
                    lat='lat', lon='lng',
                    zoom=10,
                    title="No Bus Stop Data Available"
                )
            
            # Clean data
            clean_df = bus_stops_df.dropna(subset=['lat', 'lng']).copy()
            clean_df = clean_df[(clean_df['lat'] != 0) & (clean_df['lng'] != 0)]
            
            # Debug info
            st.write(f"📊 Showing {len(clean_df)} bus stops")
            
            # Ensure required columns
            if 'risk_category' not in clean_df.columns:
                clean_df['risk_category'] = 'Medium'
            if 'estimated_temp' not in clean_df.columns:
                clean_df['estimated_temp'] = 85.0
            
            clean_df['risk_category'] = clean_df['risk_category'].fillna('Medium')
            clean_df['estimated_temp'] = clean_df['estimated_temp'].fillna(85.0)
            clean_df['StopDesc'] = clean_df['StopDesc'].fillna('Bus Stop')
            ...
            fig = px.scatter_mapbox(
                ...
                hover_name='StopDesc',
                hover_data=['estimated_temp'],
                title=f'Bus Stop Heat Risk - {time_period.upper()}',
            ...
            st.success("✅ Bus stops map created successfully")
            return fig
            
        except Exception as e:
            st.error(f"Bus stops map failed: {str(e)}")
            # Return minimal fallback map
            return px.scatter_mapbox(
                pd.DataFrame({'lat': [35.19], 'lng': [-80.79], 'label': ['Charlotte']}),
                lat='lat', lon='lng', hover_name='label',
                zoom=10,
                title="Map Error - Showing Charlotte Center"
            )
...
with tab2:
    ...
    with col2:
        if st.button("🔄 Refresh Bus Data", help="Reload bus stops from API"):
            if 'bus_stops_loaded' in st.session_state:
                del st.session_state.bus_stops_loaded
                del st.session_state.bus_stops_data
            st.rerun()
    
    analyzer = BusStopsTemperatureAnalyzer()
    
    # Use session state to avoid constant reloading
    if 'bus_stops_loaded' not in st.session_state:
        with st.spinner("Loading bus stops data..."):
            st.session_state.bus_stops_data = analyzer.fetch_bus_stops()
            st.session_state.bus_stops_loaded = True
    
    bus_stops_df = st.session_state.bus_stops_data
    ...
    if not bus_stops_df.empty:
        ...
        if census_data_for_stops:
            ...
            # Display map with census background
            try:
                fig_bus_map = analyzer.create_bus_stops_map_v2(bus_stops_with_risk, bus_time_period, census_data_for_stops)
                st.plotly_chart(fig_bus_map, use_container_width=True)
            except Exception as e:
                st.error(f"Bus stops map failed: {str(e)}")
                st.write("Showing simple fallback map...")
                
                # Simple fallback map
                fallback_fig = px.scatter_mapbox(
                    pd.DataFrame({'lat': [35.19], 'lng': [-80.79], 'name': ['Charlotte Center']}),
                    lat='lat', lon='lng', hover_name='name',
                    mapbox_style="open-street-map",
                    zoom=10,
                    title="Fallback Map - Bus stops function failed"
                )
